...
private struct PrimaryButtonBody: View {
    ...
    var body: some View {
        configuration.label
            ...
            // The one filled button on a screen is a decision, so it gets the one haptic
            // a button plays by default. Fires on the press, not the release, which is
            // when a physical button clicks. See `Haptic` for the rule.
            .haptic(.tap, trigger: configuration.isPressed) { _, isPressed in isPressed && isEnabled }
    }
}
