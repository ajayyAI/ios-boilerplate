...
///
/// Values are stored as JSON `Data` for the same reason as `UserDefaultsKeyValueStore`:
/// one code path for every `Codable` shape.
final nonisolated class KeychainKeyValueStore: KeyValueStore, Sendable {
    ...
    func set<Value: Codable>(_ value: Value, forKey key: StorageKey<Value>) {
        ...
        // `AfterFirstUnlock`: without it the item is unreadable while the device is
        // locked, which silently breaks any background work that needs it.
        // `ThisDeviceOnly`: the item stays out of iCloud and encrypted backups, so a
        // token issued to this device is never restored onto another one.
