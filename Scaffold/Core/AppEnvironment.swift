...
nonisolated struct AppEnvironment: Sendable {
    ...
    let featureFlagClient: any FeatureFlagClient
    ...
    init(config: AppConfig) {
        self.config = config
        let postHog = Self.makePostHogClient(config: config)
        analyticsClient = postHog ?? NoOpAnalyticsClient()
        featureFlagClient = postHog ?? NoOpFeatureFlagClient()
    ...
    private static func makePostHogClient(config: AppConfig) -> PostHogAnalyticsClient? {
        guard let apiKey = config.postHogAPIKey else { return nil }
