...
The SwiftUI foundation for premium iOS apps. `scripts/rename-project.py` renames the project.
...
Haptics come from `Haptic` (`.haptic(.success, trigger:)`), never from `UIImpactFeedbackGenerator` directly. A haptic marks a state change or a decision, not a tap on a link or a scroll. The primary button already plays one; do not add another to it.
