...
struct AppEnvironmentTests {
    ...
    @Test func `no keys selects the no-op feature flag client`() {
        let environment = AppEnvironment(config: Self.unconfigured)

        #expect(environment.featureFlagClient is NoOpFeatureFlagClient)
    }
    ...
    @Test func `a PostHog key serves flags and analytics from the same instance`() {
        ...
        let analytics = environment.analyticsClient as? PostHogAnalyticsClient
        let flags = environment.featureFlagClient as? PostHogAnalyticsClient
        #expect(analytics != nil)
        #expect(analytics === flags)
    }
