...
├── Core/
│   ├── Analytics/          AnalyticsClient, PostHog
│   ├── Configuration/      AppConfig from xcconfig via Info.plist; BuildEnvironment
│   ├── CrashReporting/     CrashReporter, Sentry, the report(_:context:) funnel
│   ├── DI/                 the Factory container
│   ├── Device/
│   ├── Diagnostics/        Log — os.Logger per category
│   ├── Errors/
...
│       ├── DesignSystem/   Theme, button styles, card, haptics
│       └── ViewModifiers/
├── Assets.xcassets
├── Info.plist
...
The layout above scales to a few dozen features without change. Two things become
worth doing once a team or a build time demands them, and not before:

- **Local Swift packages** for `Core/` and `Shared/` once a second target (a widget,
  an App Clip) needs them or incremental builds drag. File-system-synchronized groups
  already remove the merge-conflict reason to reach for Tuist or XcodeGen.
- **A router** once there are more than a handful of screens or a deep link to
  honour. See [`recipes/deep-links.md`](recipes/deep-links.md).
