...
| `.haptic(_:trigger:)` | Plays one of five named haptics when a value changes |

Both button styles carry press feedback that respects Reduce Motion, and dim when disabled.

## Haptics

Five words, each tied to a meaning: `.tap`, `.selection`, `.success`, `.warning`,
`.error`. A screen says what happened; `Haptic` decides how it feels.

```swift
.haptic(.success, trigger: viewModel.state.isEntitled)
.haptic(.error, trigger: viewModel.state) { _, new in new.isFailure }
```

**A haptic marks a state change or a decision.** A goal reached, a purchase completed,
a destructive confirm, a picker moving under the finger. Not a tap on a navigation
link, not a scroll, not a card appearing. The primary button plays `.tap` on press
because the one filled button on a screen is a decision; nothing else plays one by
default. Fire on every tap and haptics become noise the user learns to ignore.

Built on `sensoryFeedback`, so it follows the system haptics setting and does nothing
on hardware without a Taptic Engine.
...
│   ├── Card.swift           .card()
│   └── Haptics.swift        Haptic and .haptic(_:trigger:)
├── Components/
│   └── AsyncButton.swift
└── ViewModifiers/
    └── ErrorAlert.swift
